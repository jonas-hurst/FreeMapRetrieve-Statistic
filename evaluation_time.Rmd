...
plt <- grid.arrange(
  ggplot(filter(time_by_polygon_large, method=="P2F"), aes(x=state_number, y=mean, ymin=mean.min, ymax=mean.max)) +
  ...
  ggplot(filter(time_by_polygon_large, method=="F2P"), aes(x=state_number, y=mean, ymin=mean.min, ymax=mean.max)) +
...
ggsave(plot=plt, "./graphs/learn_time_large.png", width=11.5, height=9, units = "cm")
plot(plt)
...
plt2 <- grid.arrange(
  ggplot(filter(time_by_polygon_small, method=="P2F"), aes(x=state_number, y=mean, ymin=mean.min, ymax=mean.max, group=method, color=method)) +
    geom_point(color=p2f_color) +
    geom_line(color=p2f_color) +
    geom_errorbar(aes(width=0.2), color=p2f_color) +
    ...
    ylab("retrieve time (s)") +
    theme(aspect.ratio = 2),
  ggplot(filter(time_by_polygon_small, method=="F2P"), aes(x=state_number, y=mean, ymin=mean.min, ymax=mean.max)) +
    geom_point(color=f2p_color) +
    geom_line(color=f2p_color) +
    geom_errorbar(aes(width=0.2), color=f2p_color) +
    ylim(scale_small_min, scale_small_max) +
...
ggsave(plot=plt2, "./graphs/learn_time_small.png", width=11.5, height=9, units = "cm")
plot(plt2)
```
