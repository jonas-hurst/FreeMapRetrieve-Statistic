...
plt <- grid.arrange(
  ggplot(filter(slips_by_polygon_large, method=="P2F"), aes(x=state_number, y=mean, ymin=mean.min, ymax=mean.max)) +
    geom_point(color=p2f_color) +
    geom_line(color=p2f_color) +
    geom_errorbar(aes(width=0.2), color=p2f_color) +
    ylim(scale_large_min, scale_large_max) +
    scale_x_continuous(breaks = 1:3) +
    xlab("state number (P2F)") +
    ylab("slips") +
    theme(aspect.ratio = 2),
  ggplot(filter(slips_by_polygon_large, method=="F2P"), aes(x=state_number, y=mean, ymin=mean.min, ymax=mean.max)) +
    geom_point(color=f2p_color) +
    geom_line(color=f2p_color) +
    geom_errorbar(aes(width=0.2), color=f2p_color) +
    ylim(scale_large_min, scale_large_max) +
    scale_x_continuous(breaks = 1:3) +
    theme(axis.title.y = element_blank(), axis.ticks.y = element_blank(), axis.text.y = element_blank(), aspect.ratio = 2) +
    xlab("state number (F2P)"),
  ncol=2
)
ggsave(plot=plt, "./graphs/learn_slips_large.png", width=11.5, height=9, units = "cm")
plot(plt)
...
plt2 <- grid.arrange(
  ggplot(filter(slips_by_polygon_small, method=="P2F"), aes(x=state_number, y=mean, ymin=mean.min, ymax=mean.max, group=method, color=method)) +
    geom_point(color=p2f_color) +
    geom_line(color=p2f_color) +
    geom_errorbar(aes(width=0.2), color=p2f_color) +
    ...
    ylab("slips") +
    theme(aspect.ratio = 2),
  ggplot(filter(slips_by_polygon_small, method=="F2P"), aes(x=state_number, y=mean, ymin=mean.min, ymax=mean.max)) +
    geom_point(color=f2p_color) +
    geom_line(color=f2p_color) +
    geom_errorbar(aes(width=0.2), color=f2p_color) +
    ylim(scale_small_min, scale_small_max) +
...
ggsave(plot=plt2, "./graphs/learn_slips_small.png", width=11.5, height=9, units = "cm")
plot(plt2)
```
